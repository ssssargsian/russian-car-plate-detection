...
Проект реализуется в рамках компании, для открытия автоматических ворот

# Задача
Существует предприятие, интересуют автомобильные номера. Есть камера, она шлет поток, мы из него выделяем фото. Распознаем:
<ul>
